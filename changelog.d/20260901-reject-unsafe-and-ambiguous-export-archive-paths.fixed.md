...
allowing path traversal. Asset manifest export paths are validated before any
bytes are transferred, so an unsafe path is rejected even when the transfer
itself cannot run. ``psynet export`` publishes only ``database/`` layouts;
legacy ``data/`` zips remain valid for ``psynet load`` and ``--archive``.
