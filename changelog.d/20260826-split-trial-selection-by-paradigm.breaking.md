...
``select_chain``, and ``custom_chain_filter``; static trial makers use
``find_nodes``, ``select_node``, and ``custom_node_filter``. Selection
hooks receive a nonempty eligible list and may return the selected value
or ``Selection(value, context)``. ``get_trial_class`` must return a trial
