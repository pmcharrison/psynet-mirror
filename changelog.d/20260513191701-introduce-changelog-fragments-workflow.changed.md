...
- Added `docs/scripts/build_changelog.py` with three commands: a default rebuild that renders the `## Unreleased` block from fragments, `--migrate-unreleased` (one-shot migration of the existing manual entries), and `--release <version> <date>` (cuts a versioned section, clears `## Unreleased`, and deletes consumed fragments).
- Added a `--new <category> "<description>"` helper that creates a uniquely-named `<YYYYMMDDHHMMSS>-<slug>.<category>.md` fragment with a stub for editing, so contributors don't have to think about filename rules.
