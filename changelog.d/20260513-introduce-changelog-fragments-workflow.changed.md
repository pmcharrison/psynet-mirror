...
- Added `docs/scripts/build_changelog.py` with commands to rebuild the `## Unreleased` block from fragments, create new fragments with `--new`, and cut releases with `--release <version> <date>` (which inserts a versioned section, clears `## Unreleased`, and deletes consumed fragments for beta, release-candidate, and stable releases; alpha versions keep fragments until the first release candidate or stable release, and stable releases consume matching release-candidate sections plus any remaining fragments).
