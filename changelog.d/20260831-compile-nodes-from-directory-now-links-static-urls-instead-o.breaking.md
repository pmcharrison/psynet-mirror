...
``self.definition["url"]`` (or your ``url_key``) to prompts. The old
``asset_label`` argument was the CachedAsset dictionary key; ``url_key`` is
the definition key for that URL. Media extensions are matched
case-insensitively.
