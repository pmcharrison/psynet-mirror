...
3. Remove any temporary test skips guarding the Git reference (search
   the test suite for uses of `_get_dallinger_dependency_reference`,
   e.g. in `tests/test_check_constraints_versions.py`).
...
- Refresh the vendored constraints snapshot with
  `psynet dev ci update-dallinger-constraints`.
