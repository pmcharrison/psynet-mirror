...
Follow `power-analysis/SKILL.md` for the standard `audit/power/` files, costing,
...
parameter values in `audit/power/config.toml`. Keep the definition of each
estimand, its truth, its estimator, and its resampling rule together in
`audit/power/core.py`; a
...
fit-failure count, and precision decision in `audit/power/results.csv`. Confirm that
