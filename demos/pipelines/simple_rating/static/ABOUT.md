...
playback; pass the `/static/...` URL to `AudioPrompt`, `VideoPrompt`, or
`MediaSpec`. Gitignore large binaries if you like — `deploy.toml` still
includes `static/` (except generated `static/assets`).

Use the asset system for files created during the experiment, especially
participant recordings (`AudioRecordControl` / `VideoRecordControl` do this
for you).
