...
from psynet.utils import get_logger, get_language
...
domain_name = os.path.basename(__file__)[:-3] # strip ".py" extension, this returns the name of the file e.g. "experiment"
...
class Exp(psynet.experiment.Experiment):
    timeline = Timeline(
        ...
        InfoPage(
            Markup(
            '<h2>' + f"You have chosen to translate this experiment from English (en) to {LANGUAGE}" + '</h2>' +
            '<hr>'+
                '<p>'+
        ...
        InfoPage(
            Markup(
            '<h2>' + f"Translation imported from another file" + '</h2>' +
            '<hr>'+
                '<p>'+
        ...
        ModularPage(
            "modular_translation",
            prompt=_("You can also translate text in push buttons or any kind of page!"),
