...
def midi_to_abc(midi, duration=4):
    ...
    respelled_pitches = music21.analysis.enharmonics.EnharmonicSimplifier(
        pitch_list
    ).bestPitches()
    ...
    pitch_strings = [
        ...
        + "," * octave_down
        + f"{duration}"
        for sharp, natural, flat, letter, octave_up, octave_down in zip(
            sharps, naturals, flats, letters, octaves_up, octaves_down
        )
    ]
