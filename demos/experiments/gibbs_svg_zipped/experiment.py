...
from . import custom_synth_batch
...
class DemoGibbsSvgZippedCustomTrial(HtmlGibbsTrial):
...
class DemoGibbsSvgZippedCustomNode(HtmlGibbsNode):
    ...
    def synth_function(self, vector, output_path, chain_definition):
        custom_synth_batch.synth_batch(vector, output_path, {})


trial_maker = HtmlGibbsTrialMaker(
    id_="gibbs_svg_zipped_demo",
    trial_class=DemoGibbsSvgZippedCustomTrial,
    node_class=DemoGibbsSvgZippedCustomNode,
    ...
    start_nodes=lambda: [
        DemoGibbsSvgZippedCustomNode(context={"target": target}) for target in TARGETS
    ],
