"""Audit the demo tree for the planned minimal-demo migration."""
...
MINIMAL_KEEP_ROOT_FILES = {"README.md", "requirements.txt", "constraints.txt"}
...
def audit_demo_directory(demo_dir: str | Path) -> DemoAuditRecord:
    ...
    for entry in sorted(demo_dir.iterdir()):
        ...
        if entry.name in MINIMAL_KEEP_ROOT_FILES:
            ...
            if entry.name == "README.md":
                # Not reachable because README is preserved above, but kept for clarity.
                pass
            elif entry.name in EXPERIMENT_SCAFFOLD_GENERATED_FILES:
...
def _print_text_report(records: list[DemoAuditRecord]) -> None:
    ...
    print(f"Already minimal: {already_minimal}")
    print(f"Still full/non-minimal: {total - already_minimal}")
    ...
    for record in records:
        ...
        print(
            f"{record.path}: "
            f"{'minimal' if record.already_minimal else 'full'}; "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit PsyNet demos for the planned minimal-demo migration."
    )
    parser.add_argument(
        "--demo",
        action="append",
    ...
    parser.add_argument(
        "--only-nonminimal",
        action="store_true",
        help="Only show demos that are not already minimal.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
