...
test(
  "wait_while preserves the submitted page and wakes after async work",
  { tag: "@both" },
  async ({ page, context }) => {
      ...
      async (experimentPage) => {
        ...
        await experimentPage.addInitScript(() => {
          ...
          window.addEventListener("timelineHoldWakeReceived", () => {
            const count = Number(
              sessionStorage.getItem("timelineHoldWakeCount"),
            );
        ...
        await expect
          .poll(() =>
            experimentPage.evaluate(() => window.beforeUnloadOperations.at(-1)),
          )
          .toBe("add");
        expect(
        ...
        const busyHoldEffects = await experimentPage.evaluate(async () => {
          ...
          await psynet.handleBusyResponse(request, {
            timelineHoldResume: true,
          });
          psynet.alert = originalAlert;
          psynet.response.enable = originalResponseEnable;
...
test(
  "timeline hold restores its accessible fallback after refresh",
  { tag: "@both" },
  async ({ page, context }) => {
    const experimentDir = path.resolve(
      "tests/playwright/experiments/timeline_hold",
    ...
    await withExperiment(
      ...
      async (experimentPage) => {
        ...
        const indicator = experimentPage.locator(
...
test(
  "timeline hold uses the authoritative server timeout",
  { tag: "@both" },
  async ({ page, context }) => {
    const experimentDir = path.resolve(
...
test(
  "timeline hold preserves a reload-required page until release",
  { tag: "@both" },
  async ({ page, context }) => {
    const experimentDir = path.resolve(
    ...
    await withExperiment(
      ...
      async (experimentPage) => {
        ...
        await expect
          .poll(
            () => experimentPage.evaluate(() => typeof window.holdReloadMarker),
            { timeout: STEP_TIMEOUT_MS },
          )
          .toBe("object");
...
test(
  "timeline hold preserves same-session page identity",
  { tag: "@both" },
  async ({ page, context }) => {
    const experimentDir = path.resolve(
...
test(
  "trial feedback processing uses an in-place timeline hold",
  { tag: "@both" },
  async ({ page, context }) => {
    const experimentDir = path.resolve(
