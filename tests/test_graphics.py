...
def test_image():
    ...
    assert img.js_init == ["this.raphael = paper.image(psynet.image['logo_src'].url, 50, 50, 76, 25);"]
...
def test_validate_object_ids():
    ...
    assert str(e.value) == "in Graphic prompt, Frame 1, tried to override persistent object 'text_2'"
...
def test_validate_prompt():
    ...
    assert str(e.value) == "if prevent_control_response == True, then at least one frame must have activate_control_response == True"
    ...
    assert str(e.value) == "if prevent_control_submit == True, then at least one frame must have activate_control_submit == True"
    ...
    prompt = GraphicPrompt(
        ...
        frames=[
            Frame([
                Text("text1", "Text 1", x=150, y=30),
                Text("text2", "Text 2", x=150, y=70)
            ], activate_control_response=True, activate_control_submit=True)
        ],
        prevent_control_response=True,
        prevent_control_submit=True
    )
