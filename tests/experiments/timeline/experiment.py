...
class Exp(psynet.experiment.Experiment):
    ...
    def run_bot(self, bot):
        bot.run_until(lambda page: hasattr(page, "prompt") and "the end of the experiment!" in str(page.prompt.text))
        page = bot.get_current_page()
        text = as_plain_text(page.prompt.text)

        assert text == 'That\'s the end of the experiment! You will receive a reward of **$0.36** for the time you ' \
