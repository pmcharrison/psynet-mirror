...
def exp_dir(root):
    os.chdir(os.path.join(os.path.dirname(__file__), "../demos/demography/gmsi_two_modules_with_subscales"))
    yield
    os.chdir(root)
...
class TestExp(object):
    ...
    def test_exp_selenium(self, bot_recruits, db_session):
        for i, bot in enumerate(bot_recruits):
            ...
            assert set(list(modules["gmsi_1"].keys())) == {"time_started", "time_finished"}
            assert set(list(modules["gmsi_2"].keys())) == {"time_started", "time_finished"}
