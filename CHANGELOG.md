...
## Fixed
- Added automatic check during Docker deployment to detect missing or outdated Dockerfile format (PsyNet base image) and provide clear guidance to run `psynet update-scripts`. Dockerfiles are now mandatory for all Docker deployments. Error messages now warn users to commit pending changes before running `psynet update-scripts` and explain that it updates multiple files. Refactored error messages to avoid duplication (author: Cursor, reviewer: Peter Harrison)
