...
- Marked the highest stable release as `"preferred": true` in `docs/_static/version_switcher.json`, so the pydata-sphinx-theme version switcher always preselects the latest stable as the canonical version and renders a "you're not viewing the latest stable, switch to vX.Y" banner on alpha, rc, and older stable subdirs (author: Frank Höger).
- Refactored the docs `pages:` GitLab CI job: extracted the inline shell into `docs/scripts/build_pages.sh` (parameterised on `CI_COMMIT_TAG` / `CI_DEFAULT_BRANCH`, syntax-checkable with `bash -n` / `shellcheck`, runnable locally), and made the CI job a thin wrapper that supplies image, cache, artifacts, and rules. Dropped a stray `# To do: Skip tests if the changes are only in the docs directory` comment that had been sitting above the deploy-stage job. Refactored `docs/scripts/generate_version_switcher.py` to unify `get_tags()` and `get_all_local_tags()` into a single `list_local_tags()` helper, and split `get_latest_active_prerelease_tag()` into `_highest_stable_base()` and `_iter_active_prereleases()` helpers; CLI parity verified for `--print-highest-stable`, `--print-selected-stable-tags`, `--print-latest-rc-tag`, `--print-alpha-version`, and full JSON generation. The branch pipeline now also rebuilds the latest active prerelease tag into `public/rc/<tag>/` (so a pending release candidate's docs are always present, not only when its tag pipeline runs) and prunes stale `public/rc/<other>/` subdirs from the cache so the site never advertises a release candidate whose base version has since shipped as a final release (author: Frank Höger).
- Switched all audio-tooling demo dependencies from private GitLab git URLs to public PyPI releases (author: Frank Höger):
  - `repp` and `reppextension` are now installed via `repp-tapping==1.4.0`, which bundles the former `reppextension` package as a deprecated compatibility shim. The `tapping_iterated` and `tapping_memory` demos now import from the canonical `repp.extensions.iterated_tapping` namespace and keep a `warnings.catch_warnings()` block to silence the `numpy.matlib` `PendingDeprecationWarning` that the upstream module triggers at import time.
  - `sing4me` is now installed via the published `sing4me==2.0.0` PyPI release. The `vertical_processing` demo is unaffected by the new package layout because it only imports `sing4me.singing_extract` and bakes its own `SING4ME_CONFIG` into `singing_analysis.py`.
