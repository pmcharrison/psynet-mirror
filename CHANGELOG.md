...
- Updated custom Docker experiment PsyNet ref selection to prefer `CI_MERGE_REQUEST_SOURCE_BRANCH_SHA` over `CI_COMMIT_SHA`, avoiding unreachable synthetic merge SHAs in merge-request pipelines while still testing branch code (author: Peter Harrison)
- Expanded `adaptive_test_catr` from a smoke test into a page-based CAT demo with an item bank, a reusable catR engine wrapper, adaptive item selection/ability updates, and isolated engine tests; added inline guidance for users to customize the CAT workflow for their own experiments (author: Peter Harrison)
- Improved `adaptive_test_catr` runtime error reporting when R/catR is unavailable, with explicit local and Docker setup guidance surfaced in experiment startup failures (author: Peter Harrison)
- Fixed `adaptive_test_catr` catR integration to call `thetaEst`/`semTheta` with the correct catR argument conventions and preserve administered-item matrix dimensions (`drop=FALSE`), resolving local runtime failures after dependencies are installed (author: Peter Harrison)
- Simplified `adaptive_test_catr` imports to use direct package-relative loading and refactored catR subprocess error handling to distinguish missing runtime setup from CAT execution/configuration failures (author: Peter Harrison)
