...
- Stabilised flaky Playwright `video_feature.spec.js` demo by waiting for the page's automatic initial recording cycle (`trialStart -> responseEnable -> recordStart -> recordEnd`) to complete before clicking "Record from start" in the audio, single-camera, and dual-source sections; clicking mid-cycle could call `psynet.trial.restart()` and leave `responseEnable` (`once=True`) without a re-trigger so no new `recordStart` was emitted, causing `waitForTrialEvents` to time out (author: Frank Höger)
