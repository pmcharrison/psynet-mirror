...
- Added an `adaptive_test_catr` demo that implements a complete page-based CAT loop with catR via rpy2 (item bank, adaptive item selection, theta/SEM updates, stopping criteria), while keeping Docker-based R setup through `prepare_docker_image.sh` (author: Peter Harrison)
...
- Added a dedicated CI path for experiments listed in `ci/docker-build-experiments.txt`, so custom Docker experiments are built/tested with their own Dockerfiles using the branch under review; this includes regenerated constraints, PsyNet pin validation/rewriting, and improved per-experiment diagnostics (author: Peter Harrison)
- Consolidated CI orchestration into Python utilities (`run_ci_docker_command.py`, `run_ci_tests.py`, `run_docker_experiment_tests.py`, `pytest_common.py`) to reduce duplication and centralize pytest/docker invocation behavior (author: Peter Harrison)
...
- Improved `adaptive_test_catr` runtime resilience by fixing catR API calls (`thetaEst`/`semTheta`, matrix subsetting) and by distinguishing setup failures from CAT execution/configuration failures in error messages (author: Peter Harrison)
