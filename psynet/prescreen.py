...
class Geolocate(Module):
        ...
        self.events = join(
            ...
            CodeBlock(
                lambda experiment, participant: participant.var.set(
                    "cor_reported", participant.answer
                )
            ),
            USRegionOfResidence(),
            CodeBlock(
                lambda experiment, participant: participant.var.set(
                    "ror_reported", participant.answer
                )
            ),
            CodeBlock(
                lambda experiment, participant: participant.var.set(
                    "geolocate_country",
                    self.geolocate_details(client_ip=participant.client_ip_address)[
                        "country"
                    ],
                )
            ),
            ...
            CodeBlock(
                lambda experiment, participant: participant.var.set(
                    "VPN_Used", self.using_vpn(client_ip=participant.client_ip_address)
                )
            ),
            conditional(
                label="vpn_fail",
                condition=lambda experiment, participant: self.fail_if_using_VPN is True
                and participant.var.VPN_Used is True,
                logic_if_true=UnsuccessfulEndPage(),
            ),
            conditional(
                label="region_match",
                condition=lambda experiment, participant: self.fail_if_region_not_matched
                is True
                and participant.var.ror_reported != participant.var.geolocate_region,
                logic_if_true=UnsuccessfulEndPage(),
            ),
        )
        super().__init__(self.label, self.events)
    ...
    def geolocate_details(self, client_ip):
        if client_ip == "127.0.0.1":  # running localhost
            return {"country": "US", "region": "NYC"}  # arbitrary

        handler = ipinfo.getHandler(self.ipinfo_token)
        details = handler.getDetails(client_ip)

        return {"country": details.country, "region": details.region}
    ...
    def using_vpn(self, client_ip):
        ...
        response = urllib.request.Request(
            "http://v2.api.iphub.info/ip/{}".format(client_ip)
        )
