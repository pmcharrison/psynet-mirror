...
PRUNE_PRESERVE_TRACKED_OPTION_HELP = (
    "Never delete paths tracked by git in this directory (including files under "
    "managed directories such as docker/). Also removes generated static/assets "
    "and an untracked constraints.txt."
)
PRUNE_COMMAND_HELP = (
    ...
    "that is tracked by git, even under --force."
)
...
def prune_experiment_scaffold(
    *, preserve_files=None, force=False, preserve_tracked=False
    ...
    preserve_tracked :
        ...
        (a managed directory is preserved if any tracked path lives under it).
        Also removes generated ``static/assets`` and an untracked
        ``constraints.txt``. Intended for restoring authored-only in-repo demo
        trees after tests. Raises ``click.UsageError`` if git tracked paths
        cannot be listed.
    """
