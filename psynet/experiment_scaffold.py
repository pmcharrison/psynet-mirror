...
PRUNE_FORCE_OPTION_HELP = (
    "Remove unrecognized scaffold paths without checking their contents."
)
PRUNE_PRESERVE_TRACKED_OPTION_HELP = (
    "Never delete paths tracked by git in this directory. A managed directory "
    "such as docker/ is kept wholesale if any nested path is tracked. Also "
    "removes generated static/assets and an untracked constraints.txt."
)
PRUNE_COMMAND_HELP = (
    "Remove scaffold-managed boilerplate files from the experiment directory.\n\n"
    "Paths that differ from current PsyNet templates, including customized "
    "config.txt and README.md, are preserved by default because they may be "
    "customized or generated by another PsyNet version. --force removes those "
    "paths without checking their contents. --preserve-tracked keeps any path "
    "that is tracked by git (a managed directory is kept if any nested path is "
    "tracked), even under --force."
)
...
def prune_experiment_scaffold(
    *, preserve_files=None, force=False, preserve_tracked=False
    ...
    preserve_tracked :
        ...
        A managed directory (for example ``docker/``) is preserved wholesale if
        any tracked path lives under it. Also removes generated
        ``static/assets`` and an untracked ``constraints.txt``. Intended for
        restoring authored-only in-repo demo trees after tests. Raises
        ``click.UsageError`` if git tracked paths cannot be listed.
    """
    preserve_files = set(preserve_files or [])
    tracked = (
        _git_tracked_paths(Path("."), required=True) if preserve_tracked else set()
    )
    ...
    preserved_tracked = sorted(
        relative_path
        for relative_path in managed_paths
        if _path_is_tracked_or_has_tracked_contents(relative_path, tracked)
    ...
    if preserve_tracked:
        # static/assets is runtime-generated and must not be git-tracked; always
        # remove it when restoring authored-only trees.
...
def run_scripts_prune(*, force=False, preserve_tracked=False):
    ...
    return prune_experiment_scaffold(force=force, preserve_tracked=preserve_tracked)
...
def _git_tracked_paths(root: Path, *, required: bool = False) -> set[str]:
    ...
    try:
        ...
        if required:
            raise click.UsageError(
                "--preserve-tracked requires a working git repository in this "
                "directory, but listing tracked files failed."
            ) from exc
        return set()
