...
def join(*args):
    class JoinComponentError(TypeError):
        pass
    ...
    def normalize_component(component):
        ...
        if isinstance(component, EltCollection):
            return normalize_component(component.resolve())
        if isinstance(component, (list, tuple)):
            normalized = []
            for item in component:
                normalized.extend(normalize_component(item))
            return normalized
        if isinstance(component, Elt):
            return [component]
        raise JoinComponentError(
            "Components passed to join() must be Assets/Elts/EltCollections, "
            "functions, or lists/tuples of these."
        )

    normalized = []
    for i, arg in enumerate(args):
        try:
            normalized.extend(normalize_component(arg))
        except JoinComponentError as exc:
            raise TypeError(
                f"Element {i + 1} of the input to join() was neither an Asset/Elt/EltCollection, "
                f"a function, nor a list/tuple of such objects: ({arg})."
            ) from exc
