...
def estimate(mode):
    ...
    if mode in ["bonus", "both"]:
        maximum_bonus = experiment_class.timeline.estimate_time_credit().get_max("bonus", wage_per_hour=experiment_network.wage_per_hour)
        dallinger_log(f"Estimated maximum bonus for participant: ${round(maximum_bonus, 2)}.")
    if mode in ["time", "both"]:
        completion_time = experiment_class.timeline.estimate_time_credit().get_max("time", wage_per_hour=experiment_network.wage_per_hour)
