...
def scripts_prune(force):
    ...
    ``README.md`` and ``config.txt`` are preserved. Other paths that differ from
    current PsyNet templates are preserved by default because they may be
    customized or generated by another PsyNet version. ``--force`` removes those
    paths without checking their contents.
    """
