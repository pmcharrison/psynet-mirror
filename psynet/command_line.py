...
def export_(
    ...
    ::
        ...
        ├── regular/
        │   ├── database.zip
        │   ├── basic_data.json OR basic_data.zip
        │   ├── data/
        │   └── assets/
        └── anonymous/
            ├── database.zip
    ...
    regular/:
        Contains non-anonymized data:
            ...
            - basic_data.zip if basic data was returned as DataFrames
...
def export_basic_data(export_path, anonymize):
    ...
    subdir = "anonymous" if anonymize else "regular"
    basic_data_dir = os.path.join(export_path, subdir)
    if _is_dataframe_dict(data):
        basic_data_path = os.path.join(basic_data_dir, "basic_data.zip")
        with zipfile.ZipFile(
            make_parents(basic_data_path),
            "w",
            ...
            for key, dataframe in data.items():
                ...
                zip_file.writestr(f"{str(key)}.csv", csv_buffer.getvalue())
    else:
        basic_data_path = os.path.join(basic_data_dir, "basic_data.json")
