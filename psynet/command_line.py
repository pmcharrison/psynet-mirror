...
def scripts_prune(force, preserve_tracked):
    ...
    ``config.txt`` and ``README.md``, are preserved by default because they may
    be customized or generated by another PsyNet version. ``--force`` removes
    those paths without checking their contents. ``--preserve-tracked`` keeps
    any path that is tracked by git, even under ``--force``.
    """
