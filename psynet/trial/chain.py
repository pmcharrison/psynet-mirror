...
class ChainTrialMaker(NetworkTrialMaker):
    ...
    * :class:`~psynet.trial.chain.ChainTrial`;
        ...
        *,
        ...
        if chain_type == "within":
            ...
            if isinstance(start_nodes, list):
                self.check_start_node_list(start_nodes, node_class)
            else:
                assert start_nodes is None or callable(start_nodes)
    ...
    def check_start_node_list(self, start_nodes, node_class):
        assert len(start_nodes) > 0
        if not is_list_of(start_nodes, node_class):
            wanted_class = importable_name(node_class)
            actual_class = importable_name(type(start_nodes[0]))
            raise ValueError(
                f"Problem in trial maker: "
                f"start_nodes must correspond to the class specified in TrialMaker.node_class ({wanted_class}); "
                f"instead got {actual_class} objects."
            )
    ...
    def create_networks_within(self, experiment, participant: Participant):
        if self.start_nodes:
            ...
            self.check_start_node_list(nodes, self.node_class)
