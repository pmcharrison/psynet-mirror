...
# This script installs tab autocomplete for the psynet command
...
echo "Installing PsyNet tab autocomplete for $SHELL_NAME..."
mkdir -p "$USER_BIN_DIR"

case "$SHELL_NAME" in
    bash)
        COMPLETION_FILE="$USER_BIN_DIR/.psynet-autocomplete.bash"
        RC_FILE="$HOME/.bashrc"
        SOURCE_LINE="source $COMPLETION_FILE"
        COMPLETION_INIT="complete -o default -o nospace -F _psynet psynet"
        ;;
    zsh)
        COMPLETION_FILE="$USER_BIN_DIR/.psynet-autocomplete.zsh"
        RC_FILE="$HOME/.zshrc"
        SOURCE_LINE="source $COMPLETION_FILE"
    ...
    *)
        ...
        echo "  For bash: source $USER_BIN_DIR/.psynet-autocomplete.bash"
        echo "  For zsh: source $USER_BIN_DIR/.psynet-autocomplete.zsh"
        exit 1
        ;;
esac
...
if grep -q "\.psynet-autocomplete" "$RC_FILE" 2>/dev/null; then
    echo "Completion already installed in $RC_FILE."
else
    # Add completion to shell configuration
    echo "" >> "$RC_FILE"
    echo "# PsyNet tab autocomplete" >> "$RC_FILE"
...
echo ""
echo "tab autocomplete will be available in new terminal sessions."
echo "To enable completion in your current session, run:"
echo "  source $COMPLETION_FILE"
echo ""
echo "You can then use tab autocomplete with psynet commands:"
