var draw_network = function () {
    ...
    var type_network_sort = $('#sort_networks').val();
    // Always sort by id
    ...
    if (type_network_sort !== 'network_id') {
        var values = [];
        net_structure.networks.forEach(function(n){values.push(n[type_network_sort])})
        var max_val = Math.max.apply(Math, values);
        net_structure.networks.sort(function (first, second) {
            return first[type_network_sort] - second[type_network_sort];
        });
    }
    if ($('#order-ascending').hasClass('active')) {
        net_structure.networks = net_structure.networks.reverse();
    }

    var selected_networks = [];
    var hidden_network_ids = [];
    var trialmaker_count = {};
    var max_networks_per_trialmaker = parseInt($('#max_networks_per_trialmaker').val())

    net_structure.networks.forEach(function (network) {
        var trial_maker_id = network['trial_maker_id'];
        if ($('#' + trial_maker_id)[0].checked) {
            if (trial_maker_id in trialmaker_count) {
                if (trialmaker_count[trial_maker_id] <= max_networks_per_trialmaker) {
                    trialmaker_count[trial_maker_id] += 1;
                    selected_networks.push(network);
                } else {
                    hidden_network_ids.push(network['id']);
                }
            } else {
                trialmaker_count[trial_maker_id] = 0;
                selected_networks.push(network);
            }
        } else {
            hidden_network_ids.push(network['id']);
...
$('.toggle-order').click(function () {
    $('#order-ascending').toggleClass('active');
    $('#order-descending').toggleClass('active');
    draw_network();
});

$('.trial_maker_checkbox').click(function () {
    draw_network();
});
