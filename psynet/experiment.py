...
"""
    ...
    def test_export(self):
        """
        Export and validate test data while the server is running.

        This method is called by the autogenerated `test.py` template after
        `test_experiment()`. Experiments can override it to customize export
        behavior.
        """
        from psynet.command_line import export__local

        with tempfile.TemporaryDirectory(prefix="psynet-test-export-") as export_path:
            logger.info(f"Exporting data to {export_path}...")
            Context(export__local).invoke(export__local, path=export_path)
            logger.info("Verifying export output...")
            self.test_verify_export_output(export_path)
