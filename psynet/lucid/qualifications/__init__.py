...
def _lucid_dict_printer(d):
    out = "{ \n"
    INDENT = "  "
    for key, question_dict in d.items():
        out += f'{INDENT}"{key}": ' + "{\n"
        out += f'{INDENT*2}"question": _p("{key}", "{question_dict["question"]}"),\n'
        out += f'{INDENT*2}"options": [\n'
        for option in question_dict["options"]:
            out += f'{INDENT*3}_p("{key}", "{option}"),\n'
        out += f"{INDENT*2}],\n"
        out += f"{INDENT}}},\n"
    out += "}"
    return out
...
def prepare_qualifications():
    ...
    with open(f"{here}/questions.py", "w") as f:
        ...
        out += "_, _p = get_translator('en')\n"
        out += f"CUSTOM_QUALIFICATIONS_LUCID = {_lucid_dict_printer(question_dict)}"
        f.write(out)
