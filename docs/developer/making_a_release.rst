...
2. Prepare the release commits on the release branch:
   ...
   * Update experiment-script templates with
     ``psynet dev experiments update`` (image tags and related template
     files under ``psynet/resources/experiment_scripts/``). Bundled demos
     are authored files and are not regenerated by this command.
