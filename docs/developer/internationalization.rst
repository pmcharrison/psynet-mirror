...
- Experiments are associated with a language, but the language variable is assigned to the participant. The idea behind is that you can principly have multilingual experiments, but this is not supported by PsyNet. However, it is possible to have a multilingual experiments or people can potentially switch between languages during the experiment.
