...
When we deploy the pipeline, PsyNet handles tedious logistic details such as
copying ``static/`` media into the experiment image
...
3. Copy your stimuli into the ``static/`` directory
   (for example ``static/instrument_sounds/``).
...
- Normally you would not want to commit large numbers of audio files to a Git repository.
  ...
  without issues.
- We call the audio files in these experiments stimuli.
  Put pregenerated playback files in ``static/`` and pass ``/static/...`` URLs
  to prompts. PsyNet's asset system is for recordings and other files created
  during the experiment; see the dedicated
