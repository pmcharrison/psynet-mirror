...
* :attr:`~psynet.experiment.Experiment.var.hard_max_experiment_payment` which sets a hard, absolute limit on the amount spent in an experiment. Rewards are not paid from the point the value is reached and the amount of unpaid reward is saved in the participant's `unpaid_reward` variable. Additionally, the experimenter is notified about the issue. Default: `1100.0`.
