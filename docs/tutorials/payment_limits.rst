...
* :attr:`~psynet.experiment.Experiment.var.hard_max_experiment_payment` which sets a hard, absolute limit on the amount spent in an experiment. Bonuses are not paid from the point the value is reached and the withheld amount is stored as `planned_bonus` with bonus status `capped`. Additionally, the experimenter is notified about the issue. Default: `1100.0`.
